...
console.log('parent', parent.bar);
console.log('child', child);
// child  parent 本身都是响应式数据
//child.bar 会导致副作用函数被收集  但是child身上没有bar 会去原型身上找，parent.bar 也会收集副作用函数
//所以 child.bar 和 parent.bar 都与副作用函数简历了响应联系

//child.bar = 2
//会触发 child 的set拦截函数  但bar不存在，会去原型调用原型的set
//所以 child会触发set  parent 也会触发set
